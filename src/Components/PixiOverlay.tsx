...
export default function PixiOverlay({ segments, intersections, playerX, playerZ, detectRadius, onMovePlayer, sidebarOpen, mapRef, mapBounds }: Props) {
  ...
  // ── Draw lines & intersections (only when data changes) ──
  useEffect(() => {
    ...
    // Major lines — thick enough to be visible at moderate zoom
    const major = new Graphics();
    for (const s of segments.filter(s => s.color === 'major')) {
      major.moveTo(s.x1, s.z1); major.lineTo(s.x2, s.z2);
    }
    major.stroke({ width: 4, color: 0xc8a0ff, alpha: 0.7 });
    major.filters = [new BlurFilter({ strength: 0.3, quality: 2 })];
    ...
    for (const s of segments.filter(s => s.color === 'local')) {
      ...
      local.stroke({ width: 2, color: 0x64d2d8, alpha: 0.6 * s.alpha });
    }
    ...
    for (const int of intersections) {
      dots.circle(int.x, int.z, 12);
      dots.fill({ color: 0xf0e8ff, alpha: 0.9 });
      dots.circle(int.x, int.z, 6);
      dots.fill({ color: 0xffffff, alpha: 1 });
    }
    dots.filters = [new BlurFilter({ strength: 0.25, quality: 2 })];
    world.addChild(dots);
